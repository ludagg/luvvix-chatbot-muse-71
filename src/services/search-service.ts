    ...
    try {
      ...
      if (data.organic_results) {
        data.organic_results.forEach((result: any, index: number) => {
          results.push({
            ...
            url: result.link || '#',
            source: result.displayed_link ? new URL(result.link).hostname : 'Web',
            timestamp: new Date(),
      ...
      if (data.answer_box) {
        results.unshift({
          ...
          source: data.answer_box.displayed_link ? new URL(data.answer_box.link).hostname : 'Featured',
          timestamp: new Date(),
      ...
      if (data.video_results) {
        data.video_results.slice(0, 5).forEach((video: any, index: number) => {
          results.push({
            ...
            thumbnail: video.thumbnail?.thumbnails?.[0]?.url || `https://i.ytimg.com/vi/${video.video_id}/mqdefault.jpg`,
            source: 'YouTube',
            timestamp: new Date(),
