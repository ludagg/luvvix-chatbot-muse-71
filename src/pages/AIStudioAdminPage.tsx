...
const AIStudioAdminPage = () => {
  ...
  // Removed token authentication requirement
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Accept all visitors as authenticated
        ...
        if (configData && configData.length > 0) {
          setConfig({
            ...configData[0],
            api_key: configData[0].api_key || "csk-enyey34chrpw34wmy8md698cxk3crdevnknrxe8649xtkjrv"
          });
        }
  ...
  const saveConfig = async () => {
    try {
      ...
      // Also save config to the edge function
      try {
        const response = await fetch('/functions/v1/setup-cerebras-config', {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || "Failed to update edge function configuration");
        }
        
        console.log("Edge function config updated successfully");
      } catch (edgeFnError) {
        console.error("Failed to update edge function config:", edgeFnError);
        // Continue with database update even if edge function update fails
      }
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="csk-enyey34chrpw34wmy8md698cxk3crdevnknrxe8649xtkjrv"
                  />
                  <p className="text-xs text-slate-500 dark:text-slate-400">
